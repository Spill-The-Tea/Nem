...
namespace {
	...
	Piece PieceMap[16] = { BLANK, WPAWN, WKNIGHT, WBISHOP, WROOK, WQUEEN, WKING, BLANK, BLANK, BPAWN, BKNIGHT, BBISHOP, BROOK, BQUEEN, BKING, BLANK };
	TBPiece TBPieceMap[12] = { W_QUEEN, B_QUEEN, W_ROOK, B_ROOK, W_BISHOP, B_BISHOP, W_KNIGHT, B_KNIGHT, W_PAWN, B_PAWN, W_KING, B_KING };
	inline Piece ToPiece(TBPiece p) { return PieceMap[p]; }
	inline TBPiece ToTBPiece(Piece p) { return TBPieceMap[p]; }
	...
	const std::string PieceToChar = "QqRrBbNnPpKk    ";
	const std::string TBPieceToChar = " PNBRQK  pnbrqk";

	int MapPawns[64];
	int MapB1H1H7[64];
	int MapA1D1D4[64];
	int MapKK[10][64]; // [MapA1D1D4][64]

	int Binomial[6][64];    // [k][n] k elements from a set of n elements
	int LeadPawnIdx[5][64]; // [leadPawnsCnt][64]
	int LeadPawnsSize[5][4];       // [leadPawnsCnt][FILE_A..FILE_D]

	// Comparison function to sort leading pawns in ascending MapPawns[] order
	bool pawns_comp(Square i, Square j) { return MapPawns[i] < MapPawns[j]; }
	int off_A1H8(Square sq) { return (sq >> 3) - (sq & 7); }
	...
	struct PairsData {
		...
		TBPiece pieces[TBPIECES];        // Position pieces: the order of pieces defines the groups
	...
	TBTable<WDL>::TBTable(const std::string& code) : TBTable() {
		...
		std::transform(sides[0].begin(), sides[0].end(), sides[0].begin(), tolower);
		...
		pieceCount = popcount(pos.ColorBB(WHITE) | pos.ColorBB(BLACK));
		hasPawns = pos.PieceTypeBB(PAWN) != EMPTY;

		hasUniquePieces = false;
		for (Color c = WHITE; c <= BLACK; ++c)
			for (PieceType pt = QUEEN; pt < KING; ++pt)
				if (popcount(pos.PieceBB(pt, c)) == 1)
					hasUniquePieces = true;

		// Set the leading color. In case both sides have pawns the leading color
		// is the side with less pawns because this leads to better compression.
		int nWP = popcount(pos.PieceBB(PAWN, WHITE));
		int nBP = popcount(pos.PieceBB(PAWN, BLACK));
		bool c = !nBP
			|| (nWP  && nBP >= nWP);

		pawnCount[0] = c ? static_cast<uint8_t>(nWP) : static_cast<uint8_t>(nBP);
		pawnCount[1] = c ? static_cast<uint8_t>(nBP) : static_cast<uint8_t>(nWP);

		sides[0] = { code.substr(code.find('K', 1)) };      // Weak
		sides[1] = { code.substr(0, code.find('K', 1)) }; // Strong

		std::transform(sides[1].begin(), sides[1].end(), sides[1].begin(), tolower);

		fenStr = "8/" + sides[0] + char(8 - sides[0].length() + '0') + "/8/8/8/8/"
			+ sides[1] + char(8 - sides[1].length() + '0') + "/8 w - - 0 10";
		Position pos2(fenStr);
		key2 = pos2.GetMaterialHash();
	}
	...
	int decompress_pairs(PairsData* d, uint64_t idx) {
		...
		uint32_t k = idx / static_cast<uint32_t>(d->span);
	...
	Ret do_probe_table(const Position& pos, T* entry, WDLScore wdl, ProbeState* result) {
		...
		bool symmetricBlackToMove = (entry->key == entry->key2 && pos.GetSideToMove());
		...
		bool blackStronger = (pos.GetMaterialHash() != entry->key);

		int flipColor = (symmetricBlackToMove || blackStronger) * 8;
		int flipSquares = (symmetricBlackToMove || blackStronger) * 070;
		int stm = (symmetricBlackToMove || blackStronger) ^ pos.GetSideToMove();
		...
		if (entry->hasPawns) {
			...
			TBPiece pc = TBPiece(entry->get(0, 0)->pieces[0] ^ flipColor);

			assert(GetPieceType(ToPiece(pc)) == PAWN);

			leadPawns = b = pos.PieceBB(PAWN, GetColor(ToPiece(pc)));
			do
			...
			tbFile = static_cast<File>(squares[0] & 7);
			if (tbFile > FileD)
				tbFile = static_cast<File>((squares[0] ^ 7) & 7); // Horizontal flip: SQ_H1 -> SQ_A1
		}
		...
		b = pos.OccupiedBB() ^ leadPawns;
		do {
			Square s = lsb(b);
			b &= b - 1;
			squares[size] = s ^ flipSquares;
			pieces[size++] = TBPiece(ToTBPiece(pos.GetPieceOnSquare(s)) ^ flipColor);
		} while (b);
		...
		if (static_cast<File>(squares[0] & 7) > FileD)
		...
		if (static_cast<Rank>(squares[0] >> 3) > Rank4)
			for (int i = 0; i < size; ++i)
		...
		if (entry->hasUniquePieces) {
			...
			// to 0...3 and finally MapB1H1H7[] maps the b1-h1-h7 triangle to 0..27.
				...
				idx = (6 * 63 + static_cast<Rank>(squares[0] >> 3) * 28
			...
			// First two pieces are on a1-h8 diagonal, third below
				...
				idx = 6 * 63 * 62 + 4 * 28 * 62
				+ static_cast<Rank>(squares[0] >> 3) * 7 * 28
				+ (static_cast<Rank>(squares[1] >> 3) - adjust1) * 28
				+ MapB1H1H7[squares[2]];

			// All 3 pieces on the diagonal a1-h8
			else
				idx = 6 * 63 * 62 + 4 * 28 * 62 + 4 * 7 * 28
				+ static_cast<Rank>(squares[0] >> 3) * 7 * 6
				+ (static_cast<Rank>(squares[1] >> 3) - adjust1) * 6
				+ (static_cast<Rank>(squares[2] >> 3) - adjust2);
		}
		else
	...
	void* mapped(TBTable<Type>& e, const Position& pos) {
		...
		for (PieceType pt = QUEEN; pt <= PAWN; ++pt) {
			w += std::string(popcount(pos.PieceBB(pt, WHITE)), PieceToChar[2 * pt]);
			b += std::string(popcount(pos.PieceBB(pt, BLACK)), PieceToChar[2 * pt]);
		}

		fname = (e.key == pos.GetMaterialHash() ? w + 'v' + b : b + 'v' + w)
			+ (Type == WDL ? ".rtbw" : ".rtbz");
	...
	Ret probe_table(const Position& pos, ProbeState* result, WDLScore wdl = WDLDraw) {
		...
		TBTable<Type>* entry = TBTables.get<Type>(pos.GetMaterialHash());
	...
	WDLScore search(Position& pos, ProbeState* result) {
		...
		{
			Move move = generatedMoves[i].move;
			if (!pos.IsCapture(move)
				&& (!CheckZeroingMoves || GetPieceType(pos.GetPieceOnSquare(from(move))) != PAWN))
				continue;
			...
			Position next(pos);
			next.ApplyMove(move);
			value = -search<false>(next, result);
...
void tablebases::init(const std::string& paths) {
	...
	for (Square s = Square::A1; s <= Square::D4; ++s)
		if (off_A1H8(s) < 0 && (s & 7) <= static_cast<int>(FileD))
			MapA1D1D4[s] = code++;

		else if (!off_A1H8(s) && (s & 7) <= static_cast<int>(FileD))
	...
	for (int idx = 0; idx < 10; idx++)
		for (Square s1 = Square::A1; s1 <= Square::D4; ++s1)
			if (MapA1D1D4[s1] == idx && (idx || s1 == Square::B1)) // SQ_B1 is mapped to 0
			{
				for (Square s2 = Square::A1; s2 <= Square::H8; ++s2)
					if ((KingAttacks[s1] | ToBitboard(s1)) & ToBitboard(s2))
	...
	for (int leadPawnsCnt = 1; leadPawnsCnt <= 4; ++leadPawnsCnt)
		for (File f = FileA; f <= FileD; ++f)
		{
			...
			{
				Square sq = static_cast<Square>(8 * static_cast<int>(r) + static_cast<int>(f));
	...
	for (PieceType p1 = PAWN; p1 >= QUEEN && p1 < KING; --p1) {
		TBTables.add({ KING, p1, KING });

		for (PieceType p2 = PAWN; p2 >= p1 && p2 < KING; --p2) {
			TBTables.add({ KING, p1, p2, KING });
			TBTables.add({ KING, p1, KING, p2 });

			for (PieceType p3 = PAWN; p3 >= QUEEN && p3 < KING; --p3)
				TBTables.add({ KING, p1, p2, KING, p3 });

			for (PieceType p3 = PAWN; p3 >= p2 && p3 < KING; --p3) {
				TBTables.add({ KING, p1, p2, p3, KING });

				for (PieceType p4 = PAWN; p4 >= p3 && p4 < KING; --p4)
					TBTables.add({ KING, p1, p2, p3, p4, KING });

				for (PieceType p4 = PAWN; p4 >= QUEEN && p4 < KING; --p4)
					TBTables.add({ KING, p1, p2, p3, KING, p4 });
			}

			for (PieceType p3 = PAWN; p3 >= p1 && p3 < KING; --p3)
				for (PieceType p4 = PAWN; p4 >= (p1 == p3 ? p2 : p3) && p4 < KING; --p4)
...
int tablebases::probe_dtz(Position& pos, ProbeState* result) {
	...
	*result = OK;
	...
	{
		Move move = generatedMoves[i].move;
		bool zeroing = pos.IsTactical(move) || GetPieceType(pos.GetPieceOnSquare(from(move))) == PAWN;
		...
		dtz = zeroing ? -dtz_before_zeroing(search<false>(next, result))
			: -probe_dtz(next, result);

		// If the move mates, force minDTZ to 1
		if (dtz == 1 && next.Checked() && next.GetResult() == Result::MATE)
			minDTZ = 1;
...
bool tablebases::root_probe(Position& pos, RootMoves& rootMoves) {
	...
	{
		Position next(pos);
		next.ApplyMove(m.pv[0]);

		// Calculate dtz for the current move counting from the root position
		if (next.GetDrawPlyCount() == 0)
		{
			// In case of a zeroing move, dtz is one of -101/-1/0/1/101
			WDLScore wdl = -probe_wdl(next, &result);
			dtz = dtz_before_zeroing(wdl);
		}
		else
		{
			// Otherwise, take dtz for the new position and correct by 1 ply
			dtz = -probe_dtz(next, &result);
			dtz = dtz > 0 ? dtz + 1
				: dtz < 0 ? dtz - 1 : dtz;
		}
		...
		m.tbScore = r >= bound ? VALUE_MATE - MAX_DEPTH - 1
			: r > 0 ? Value((std::max(3, r - 800) * int(settings::parameter.PieceValues[PAWN].egScore)) / 200)
			: r == 0 ? VALUE_DRAW
			: r > -bound ? Value((std::min(-3, r + 800) * int(settings::parameter.PieceValues[PAWN].egScore)) / 200)
			: -VALUE_MATE + MAX_DEPTH + 1;
	
		// Adjust rank, for better ordering
		if (m.tbRank == 1000) m.tbRank = 1200 - dtz + 1000 * (next.GetResult() == Result::MATE);
		else if (m.tbRank == -1000) m.tbRank = -1200 - dtz;
	}
...
//
// A return value false indicates that not all probes were successful.
bool tablebases::root_probe_wdl(Position& pos, RootMoves& rootMoves) {
...
bool tablebases::rank_root_moves(Position& pos, RootMoves& rootMoves) {
#ifdef _DEBUG
	std::sort(rootMoves.begin(), rootMoves.end(),
		[](const RootMove &a, const RootMove &b) { return a.pv[0] > b.pv[0]; });
#endif
	...
	if (Cardinality >= popcount(pos.OccupiedBB()) && !pos.GetCastles())
	{
